# Local values for commonly used configuration parameters
locals {
  http_port     = 80
  any_port      = 0
  any_protocol  = "-1"
...
resource "aws_lb_listener" "http" {
  ...
  port              = local.http_port     # Standard HTTP port
  protocol          = "HTTP"
...
data "terraform_remote_state" "db" {
  ...
  config = {
    bucket = var.db_remote_state_bucket               # Same S3 bucket
    key    = var.db_remote_state_key              # Path to database state file
